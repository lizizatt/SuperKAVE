...
inline void checkGLErrors(const char *caller)
{
...
inline std::string commonVShader(void)
{
	...
	"	L = vec3(gl_LightSource[0].position-m);\n"
	"	N = gl_NormalMatrix * gl_Normal;\n"
	"	E = vec3(0,0,0)-vec3(gl_ModelViewMatrix*gl_Vertex);\n"
	"}\n");
}
